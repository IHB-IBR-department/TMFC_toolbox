...
%% ======[ Select conditions of interest for LSS analysis via GUI ]========
function [conditions] = LSS_conditions_GUI(all_cond) 
    ...
    LSS_MW = figure('Name', 'LSS: Select conditions', 'NumberTitle', 'off', 'Units', 'normalized', 'Position', [0.45 0.25 0.22 0.56],'MenuBar', 'none','ToolBar', 'none','color','w','Resize','off','WindowStyle','modal','CloseRequestFcn', @MW_exit);
    ...
    %----------------------------------------------------------------------
    % Function to close LSS GUI without selection of conditions
    %----------------------------------------------------------------------
    function MW_exit(~,~)
        delete(LSS_MW);
        conditions = NaN;
    ...
    function MW_LB1_select(~,~)
        index = get(LSS_MW_LB1, 'Value');  
        LSS_MW_SE1 = index;      
    end

    %----------------------------------------------------------------------
    function MW_LB2_select(~,~)
        index = get(LSS_MW_LB2, 'Value');  
        LSS_MW_SE2 = index;             
    end
    ...
    %----------------------------------------------------------------------
    % Help window for LSS conditions
    %----------------------------------------------------------------------
    function MW_help(~,~)

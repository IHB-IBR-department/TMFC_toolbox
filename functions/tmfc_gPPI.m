...
for iSub = start_sub:nSub
    ...
    if tmfc.defaults.analysis == 1
        ...
        SPM = load(fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI',['Subject_' num2str(iSub,'%04.f')],tmfc.ROI_set(ROI_set_number).ROIs(1).name,'SPM.mat'));
        ...
        switch tmfc.defaults.parallel
            case 0  % Sequential
                for jROI = 1:nROI
                    ...
                    for cond_PPI = 1:nCond
                        copyfile(fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI',['Subject_' num2str(iSub,'%04.f')], ... 
                            tmfc.ROI_set(ROI_set_number).ROIs(jROI).name,['beta_' num2str(PPI_num(cond_PPI) - 1 + SPM.SPM.Sess(PPI_sess(cond_PPI)).col(1) + length(SPM.SPM.Sess(PPI_sess(cond_PPI)).U),'%04.f') '.nii']), ...
                            fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI','Seed_to_voxel',tmfc.ROI_set(ROI_set_number).ROIs(jROI).name, ...
        ...
        SPM = load(fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI',['Subject_' num2str(iSub,'%04.f')],tmfc.ROI_set(ROI_set_number).ROIs(1).name,'SPM.mat'));
        for cond_PPI = 1:nCond
            ppi_matrix = squeeze(beta(PPI_num(cond_PPI) - 1 + SPM.SPM.Sess(PPI_sess(cond_PPI)).col(1) + length(SPM.SPM.Sess(PPI_sess(cond_PPI)).U),:,:));

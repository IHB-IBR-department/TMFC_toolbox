...
% will be included in the gPPI GLMs. If the original GLMs contain time and
% dispersion derivatives, they will be removed from the gPPI GLMs.
%
...
for iSub = start_sub:nSub
    ...
    for jROI = 1:nROI
        if isdir(fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI',['Subject_' num2str(iSub,'%04.f')],tmfc.ROI_set(ROI_set_number).ROIs(jROI).name))
            rmdir(fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI',['Subject_' num2str(iSub,'%04.f')],tmfc.ROI_set(ROI_set_number).ROIs(jROI).name),'s');
        end
        mkdir(fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI',['Subject_' num2str(iSub,'%04.f')],tmfc.ROI_set(ROI_set_number).ROIs(jROI).name));
        ...
        % gPPI GLM batch
        matlabbatch{1}.spm.stats.fmri_spec.dir = {fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI',['Subject_' num2str(iSub,'%04.f')],tmfc.ROI_set(ROI_set_number).ROIs(jROI).name)};
        ...
        case 0  % Sequential
            for jROI = 1:nROI
                ...
                tmfc_parsave_batch(fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI','GLM_batches',tmfc.ROI_set(ROI_set_number).ROIs(jROI).name,...
                    ['Subject_' num2str(iSub,'%04.f') '_gPPI_GLM.mat']),batch{jROI});
            end
    ...
    if tmfc.defaults.analysis == 1
        ...
        % Seed-to-voxel
        for jROI = 1:nROI
            matlabbatch{1}.spm.stats.fmri_est.spmmat(1) = {fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI',['Subject_' num2str(iSub,'%04.f')],tmfc.ROI_set(ROI_set_number).ROIs(jROI).name,'SPM.mat')};
            ...
            case 0  % Sequential
                for jROI = 1:nROI
                    ...
                    if rWLS(iSub) == 0
                        ...
                        SPM = load(fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI',['Subject_' num2str(iSub,'%04.f')],tmfc.ROI_set(ROI_set_number).ROIs(jROI).name,'SPM.mat')).SPM;
                        SPM.xVi.form = 'wls';
                        nScan = sum(SPM.nscan);
                        for jScan = 1:nScan
                        ...
                        original_dir = pwd;
                        cd(fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI',['Subject_' num2str(iSub,'%04.f')],tmfc.ROI_set(ROI_set_number).ROIs(jROI).name));
                        tmfc_spm_rwls_spm(SPM);
                        cd(original_dir);
                        clear SPM
                    end
                  
                    SPM = load(fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI',['Subject_' num2str(iSub,'%04.f')],tmfc.ROI_set(ROI_set_number).ROIs(1).name,'SPM.mat')).SPM;
        ...
        for kSess = 1:nSess
            for jROI = 1:nROI
                VOI = fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'VOIs', ... 
                      ['Subject_' num2str(iSub,'%04.f')],['VOI_' tmfc.ROI_set(ROI_set_number).ROIs(jROI).name '_' num2str(sess_num(kSess)) '.mat']);
    ...
    rmdir(fullfile(tmfc.project_path,'ROI_sets',tmfc.ROI_set(ROI_set_number).set_name,'gPPI',['Subject_' num2str(iSub,'%04.f')]),'s');
...
function tmfc_save_gPPI_betas(tmfc,ROI_set_number,iSub,jROI,cond_PPI,PPI_num,PPI_sess,cond_list,SPM)
    ...
    orig_path = fullfile(gPPI_path,['Subject_' num2str(iSub,'%04.f')],ROI_name,['beta_' num2str(beta_number,'%04.f') '.nii']);
    new_path =  fullfile(gPPI_path,'Seed_to_voxel',ROI_name,['Subject_' num2str(iSub,'%04.f') '_Contrast_' num2str(cond_PPI,'%04.f') '_' cond_list(cond_PPI).file_name '.nii']);
